import {
    ...
    ACCOUNT_SIZE,
    MAX_TOTAL_DRAWDOWN,
    REVENGE_TRADING_DELAY_MINS,
    RISK_PER_TRADE_PERCENT
} from "../lib/clients.js";
import { isDrawdownBreached, checkRevengeTrading, calculateLotSize } from "../lib/risk.js";
import crypto from "crypto";

export async function executeMT5Trade(symbol: string, action: string, volume: number, sl: number, tp: number) {
...
export async function executeBlofinTrade(instId: string, side: string, orderType: string, size: string, price?: string, marginMode: string = "cross", positionSide: string = "net") {
    ...
    try {
        ...
        const bodyObj: any = { instId, side, orderType, size, marginMode, positionSide };
        if (price) bodyObj.price = price;
        ...
        if (result.code !== "0" && result.code !== 0) throw new Error(`Blofin Error: ${result.msg}`);
