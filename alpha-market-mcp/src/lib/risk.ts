...
export function calculateLotSize(symbol: string, currentPrice: number, sl: number, riskPercent: number, accountSize: number): number {
...
export function isDrawdownBreached(currentEquity: number, accountSize: number, maxTotalDrawdown: number): boolean {
    const totalLoss = accountSize - currentEquity;
    return totalLoss >= maxTotalDrawdown;
}

export function checkRevengeTrading(lastTradeTime: string, delayMins: number): { blocked: boolean; remainingMins: number } {
