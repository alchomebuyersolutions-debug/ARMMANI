 ...
 */
async function scrapeMarketNews(query: string, memoryTopic: string) {
    ...
    try {
        ...
        const cleanData = organicResults.map((i: any) => ({
            title: i.title,
            url: i.url,
...
async function pollTelegramSignals() {
    ...
    try {
        ...
        console.error("[Worker] Telegram Poll Error:", (error as any).message);
    }
}
