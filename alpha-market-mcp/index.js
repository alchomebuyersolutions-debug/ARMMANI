...
const server = new Server({ name: "alpha-market-mcp", version: "2.0.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        case "execute_mt5_trade":
            return await executeMT5Trade(args?.symbol, args?.action, args?.volume || 0, args?.sl || 0, args?.tp || 0);
        case "execute_blofin_trade":
            return await executeBlofinTrade(args?.instId, args?.side, args?.orderType || "market", args?.size);
        case "save_trade_memory":
            return await saveTradeMemory(args?.topic, args?.data);
        case "get_trade_memory":
            return await getTradeMemory(args?.topic);
        case "get_market_news":
            return await getMarketNews(args?.query);
        case "run_scraper":
            return await runApifyScraper(args?.url, args?.extractRules);
        case "save_strategy":
            return await saveStrategy(args?.filename, args?.content);
        default:
